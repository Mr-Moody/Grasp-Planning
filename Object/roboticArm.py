...
class RoboticArm:
    ...
    def __init__(self):
        ...
        self.BASE_STEP_SLEEP = 1.0 / 360.0  # original per-step sleep
        self.BASE_STEPS_APPROACH = 75
        self.BASE_STEPS_FAST = 30
    ...
    def move_to_pose(self, robot_id, ee_link_idx, target_pos, target_orn, steps=None):
        ...
        if steps is None:
            steps = max(10, int(self.BASE_STEPS_APPROACH / self.SIMULATION_SPEED))
        step_sleep = self.BASE_STEP_SLEEP / self.SIMULATION_SPEED
    ...
    def move_to_pose_fast(self, robot_id, ee_link_idx, target_pos, target_orn, steps=None):
        ...
        if steps is None:
            steps = max(6, int(self.BASE_STEPS_FAST / self.SIMULATION_SPEED))
        step_sleep = self.BASE_STEP_SLEEP / self.SIMULATION_SPEED
        current_pose = p.getLinkState(robot_id, ee_link_idx)
        current_pos = np.array(current_pose[0])
        current_orn = np.array(current_pose[1])
    ...
    def wait_for_end_effector(self, robot_id, ee_link_idx, target_pos, target_orn, pos_tol=0.01, orn_tol=0.05, timeout=1.0):
    ...
    def do_grasp_and_evaluate(self, robot_id, gripper_joints, ee_link_idx,
        ...
        self.move_to_pose(robot_id, ee_link_idx, grasp_center, approach_orn)
        # Ensure we've actually arrived at the approach pose before closing gripper
        reached = self.wait_for_end_effector(robot_id, ee_link_idx, grasp_center, approach_orn, pos_tol=0.008, orn_tol=0.08, timeout=1.5)
        if not reached:
    ...
    def robotic_arm_grasp_sampling(self):
